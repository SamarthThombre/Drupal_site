...
class __TwigTemplate_617a884256e8fff302657e9cdcb4c9a3 extends Template
{
 ...
 *   - tag: The HTML tag name to use; either \x27th\x27 or \x27td\x27.
 ...
 *     \x27even / odd\x27 styling. Defaults to FALSE.
    ...
    <thead>
      <tr>
        {% for cell in header %}
          {%
            set cell_classes = [
              cell.active_table_sort ? \x27is-active\x27,
            ]
          %}
  ...
  {% if rows %}
    <tbody>
      {% for row in rows %}
        {%
          set row_classes = [
            not no_striping ? cycle([\x27odd\x27, \x27even\x27], loop.index0),
          ]
        %}
