...
/* field.html.twig */
class __TwigTemplate_cff7c7cd2baec67231b43e7ada5f9210 extends Template
{
 ...
 * to your theme\x27s directory and customize it, just like customizing other
 ...
 *   - content: The field item\x27s content.
...
{%
  set classes = [
    ...
    label_display == \x27inline\x27 ? \x27clearfix\x27,
  ]
%}
{%
  set title_classes = [
    \x27field__label\x27,
    label_display == \x27visually_hidden\x27 ? \x27visually-hidden\x27,
  ]
%}

{% if label_hidden %}
  {% if multiple %}
    <div{{ attributes.addClass(classes, \x27field__items\x27) }}>
      {% for item in items %}
        <div{{ item.attributes.addClass(\x27field__item\x27) }}>{{ item.content }}</div>
      {% endfor %}
    </div>
  {% else %}
    {% for item in items %}
      <div{{ attributes.addClass(classes, \x27field__item\x27) }}>{{ item.content }}</div>
    {% endfor %}
  {% endif %}
{% else %}
...
{% endif %}
", "field.html.twig", "themes/icar_theme/templates/field/field.html.twig");
    }
