...
class __TwigTemplate_ecb1669cf1fe0d77ee3983ff54952ba9 extends Template
{
...
<select{{ attributes }}>
  {% for option in options %}
    {% if option.type == \x27optgroup\x27 %}
      <optgroup label=\"{{ option.label }}\">
        {% for sub_option in option.options %}
          <option value=\"{{ sub_option.value }}\"{{ sub_option.selected ? \x27 selected=\"selected\"\x27 }}>{{ sub_option.label }}</option>
        {% endfor %}
      </optgroup>
    {% elseif option.type == \x27option\x27 %}
      <option value=\"{{ option.value }}\"{{ option.selected ? \x27 selected=\"selected\"\x27 }}>{{ option.label }}</option>
    {% endif %}
  {% endfor %}
</select>
